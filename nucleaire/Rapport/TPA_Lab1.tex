...
\vspace{0.5em}
\subsection*{Résumé}
On ne le sent pas, mais des rayons gamma nous traversent en ce moment même: ils viennent du béton, des roches, et même de l'intérieur de notre propre corps. Cette expérience visait à étalonner un scintillateur NaI(Tl) couplé à un analyseur multicanaux (Easy-MCA-8k), à l'aide de cinq sources $\gamma$ connues (\textsuperscript{133}Ba, \textsuperscript{57}Co, \textsuperscript{60}Co, \textsuperscript{137}Cs, \textsuperscript{22}Na), puis à utiliser cet étalonnage pour identifier les sources qui forment le spectre du bruit de fond radioactif du laboratoire. Le modèle utilisé pour faire la correspondance entre les canaux de l'analyseur et l'énergie des rayons gamma a été un modèle quadratique ($R^2 = 0{,}99997$, RMSE $= 2{,}6~\text{keV}$, contre $5{,}0~\text{keV}$ pour un modèle linéaire), et la résolution du détecteur suit une loi de puissance $R = (122 \pm 15)\cdot E^{-0{,}44 \pm 0{,}03}~\%$, proche du comportement statistique attendu en $E^{-1/2}$. Sur le spectre du \textsuperscript{137}Cs, le front Compton et le pic de rétrodiffusion prédits par les mathématiques de la diffusion Compton (477,4~keV et 184,3~keV) sont exactement, plus ou moins la résolution du détecteur, les mêmes que ceux observés ($\approx 462$~keV et $\approx 195$~keV). Finalement, une acquisition de 36~minutes sans source a révélé un pic dominant à $1485 \pm 3~\text{keV}$, qui correspond à la raie de $1460{,}8~\text{keV}$ du \textsuperscript{40}K. Ceci confirme que le potassium naturel, présent dans le béton, la brique et le sol, est la principale source de radioactivité ambiante du laboratoire. Ce même principe, combiné à une analyse de la direction des vents, a même permis à la centrale nucléaire de Forsmark, en Suède, de détecter la catastrophe de Tchernobyl deux jours après qu'elle ait eu lieu \cite{forsmark}.
...
L'ensemble du traitement a été fait en Python (NumPy/SciPy), voir le script \texttt{analyse\_spectres.py} (\hyperref[sec:annexeB]{Annexe~B}). Pour chaque raie de référence, la région du spectre autour du canal attendu est d'abord localisée par une recherche de maximum local, puis ajustée par une gaussienne sur fond linéaire,
\beq{eq:gauss}
...
La Figure~\ref{fig:calib}a montre les onze points d'étalonnage ainsi que les modèles linéaire et quadratique. Le modèle quadratique,
\beq{eq:calib}
...
La résolution relative $R = \text{FWHM}/E$ de chaque photopic (Tableau~\ref{tab:peaks}), tracée à la Figure~\ref{fig:calib}b, diminue de façon monotone avec l'énergie: d'environ 28 à 34~\% pour les raies sous 35~keV, jusqu'à 4,5 à 5~\% pour les raies du \textsuperscript{60}Co au-delà de 1,1~MeV. Un ajustement en loi de puissance donne
\beq{eq:resolution}
...
\begin{table}[htbp]
\centering
\caption{Détail des onze raies d'étalonnage: énergie de référence, canal ajusté, résidu par rapport au modèle quadratique ($E_{\text{lit.}} - E_{\text{modèle}}$, même convention qu'à la Figure~\ref{fig:calib}), et résolution en énergie ($R = \text{FWHM}/E$).}
\label{tab:peaks}
\footnotesize
...
\begin{figure}[tbp]
    ...
    \begin{subfigure}[t]{0.44\textwidth}
        ...
        \caption{Énergie (littérature) en fonction du canal ajusté, pour les onze raies. Le panneau du bas montre les résidus par rapport au modèle quadratique, compatibles avec le RMSE global de 2,6~keV.}
        \label{fig:calib}
    \end{subfigure}\hfill
...
\begin{itemize}
    ...
    \item une \emph{chute abrupte} du continuum, dont le point mi-hauteur mesuré ($\approx 462~\text{keV}$) est proche du front Compton théorique (éq.~\ref{eq:compton_edge}, $477{,}4~\text{keV}$).
    \item une \emph{bosse de rétrodiffusion} mesurée vers $\approx 195~\text{keV}$, proche de la prédiction de $184{,}3~\text{keV}$ (éq.~\ref{eq:backscatter}).
\end{itemize}
...
Le scintillateur NaI(Tl) a été étalonné avec succès à l'aide de cinq sources $\gamma$ connues. Le modèle utilisé pour convertir les canaux en énergie a été un modèle quadratique, avec une précision de 2,6~keV (RMSE) sur une plage de 30~keV à 1,33~MeV. Les deux objectifs de l'expérience ont été atteints: l'instrument calibré a permis de vérifier quantitativement la physique de la diffusion Compton (front Compton et pic de rétrodiffusion du \textsuperscript{137}Cs conformes aux prédictions théoriques, à la résolution près) et d'identifier le \textsuperscript{40}K comme principale source du bruit de fond ambiant du laboratoire, avec une énergie mesurée en accord avec la valeur de référence à moins d'un écart-type. Au passage, la résolution en énergie du détecteur a été caractérisée sur plus d'un ordre de grandeur en énergie, ce qui a montré un comportement proche du régime limité par la statistique de comptage des photons de scintillation ($R \propto E^{-0{,}44}$, contre $E^{-0{,}5}$ attendu).

Un prolongement logique de cette expérience serait de refaire l'identification du bruit de fond avec un détecteur au germanium hyperpur, plus précis que le NaI(Tl). Les raies plus faibles des chaînes de désintégration de l'uranium et du thorium, actuellement en dessous du bruit statistique de nos mesures, pourraient alors devenir visibles.
...
\section*{Annexe B: script d'analyse}
\label{sec:annexeB}

L'ensemble du traitement (localisation et ajustement des pics, régression d'étalonnage, génération des figures) est implémenté dans le script \texttt{analyse\_spectres.py} (dossier \texttt{nucleaire/scripts/}) du dépôt du cours. Le script n'est pas reproduit intégralement ici (près de 500 lignes) afin de respecter la longueur cible du rapport. Il demeure disponible dans le dépôt pour consultation ou reproduction des résultats.
